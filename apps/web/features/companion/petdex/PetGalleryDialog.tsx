...
const PRIMARY_ACTION_BUTTON_CLASS =
  "rounded-[9px] bg-[var(--accent-primary)] text-[var(--text-inverse)] shadow-none hover:opacity-90 active:opacity-80 disabled:opacity-50";
...
export function PetGalleryDialog({
  ...
  const content = (
    <div
      className="petdex-dialog-overlay"
      onClick={handleRequestClose}
    >
      <div
        className="petdex-dialog-panel"
        onClick={(event) => event.stopPropagation()}
      >
        {mode === "gallery" ? (
          <div className="petdex-gallery-title-badge pointer-events-none absolute left-[14px] -top-[48px] z-10 h-[100px] w-[200px] -rotate-5">
            <img
          ...
          <div className="flex translate-x-1 translate-y-[2px] items-center gap-2 pt-2">
            {mode === "gallery" ? (
              <button
                ...
                className="petdex-toggle-btn flex h-7 items-center gap-2 rounded-full px-2.5 text-[12px] transition-colors"
              >
                <span>{t("myBuddy.companion.toggleVisibility")}</span>
                <span
                  className={cn(
                    "petdex-toggle-track relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors",
                    companionHidden ? "" : "petdex-toggle-track--on",
                  )}
                >
                  <span
                    className="petdex-toggle-knob absolute h-3.5 w-3.5 rounded-full"
                    style={{
                      transform: companionHidden ? 'translateX(3px)' : 'translateX(17px)',
            ...
            {mode === "gallery" ? (
              <Button
                ...
                variant="outline"
                size="sm"
                className="petdex-import-btn h-7 rounded-full px-2.5 text-[11px] shadow-none"
            ...
            <button
              type="button"
              onClick={handleRequestClose}
              className="petdex-close-btn flex h-7 w-7 items-center justify-center rounded-md bg-transparent transition-colors"
                    ...
                    {group.map((id) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "flex h-7 w-full min-w-0 items-center rounded-[6px] px-2 text-left text-xs bg-transparent leading-none text-slate-100 transition-colors hover:bg-white/[0.08] hover:text-white",
